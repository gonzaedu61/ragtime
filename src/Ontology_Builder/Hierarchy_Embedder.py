...
class Hierarchy_Embedder:
    ...
    def _semantic_lineage_path(
        ...
        the top `drop_levels` entries (e.g., to skip generic root categories).
        """
        if not cluster_id:
            return []

        # Build list of ancestor cluster IDs
        segments = cluster_id.split(".")
        lineage_ids = [".".join(segments[:i]) for i in range(1, len(segments) + 1)]
        ...
        # Drop the top N levels if requested
        if drop_levels > 0:
            labels = labels[drop_levels:]
