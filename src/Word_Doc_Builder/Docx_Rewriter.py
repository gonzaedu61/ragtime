...
class Docx_Rewriter:
    ...
    # ----------------------------------------------------------------------
    # Determine semantic text type
    # ----------------------------------------------------------------------
    def _get_text_type(self, paragraph, container_type: str) -> str:
    ...
    def _rewrite_paragraph(self, paragraph, metadata):
        ...
        text_type = self._get_text_type(paragraph, metadata["type"])
    ...
    def _rewrite_text(self, text: str, metadata: dict) -> str:
        prompt = self.prompt_template.format(
            text=text,
            text_type=metadata.get("text_type", "paragraph")
        )
    ...
    # ----------------------------------------------------------------------
    # Count only rewritable paragraphs (for progress bar)
    # ----------------------------------------------------------------------
    def _count_rewritable_paragraphs(self, doc):
        count = 0
        ...
        for paragraph in doc.paragraphs:
            if any(self._is_toc_run(run) for run in paragraph.runs):
                continue
            if self._should_skip_text(paragraph.text):
                continue
            count += 1
    ...
    def rewrite(self):
        ...
        if self.progress_enabled:
            total = self._count_rewritable_paragraphs(doc)
