...
class PDF_Chunker:
    ...
    def extract_spans(self) -> List[TextSpan]:
        ...
        if self.export_blocks:
            out_path = os.path.join(
                self.output_dir,
                f"{self.document_name}_blocks.json"
            )
    ...
    def group_spans_into_blocks(self, spans: List[TextSpan]) -> List[LogicalBlock]:
        ...
        def flush():
            ...
            if m:
                # Numbered heading → use numbered threshold
                if is_heading_candidate and is_short_numbered and few_lines and valid_heading_ending and same_block:
                    kind = "heading"
            else:
                # Non-numbered heading → use non-numbered threshold
                if is_heading_candidate and is_short_non_numbered and few_lines and valid_heading_ending and same_block:
                    kind = "heading"
    ...
    def parse_document(self, pdf_pathName: str) -> List[StructuralChunk]:
        ...
        if self.export_spans:
            ...
            out_path = os.path.join(
                self.output_dir,
                f"{self.document_name}_spans.json"
            )
            with open(out_path, "w", encoding="utf-8") as f:
